...
- **Bill section** — one of the five mutable sub-endpoints that, together with the identity record, make up the Bill aggregate per [ADR 0009](docs/adr/0009-multi-endpoint-entities-split-jsonl.md): `cosponsors`, `actions`, `subjects`, `titles`, `summaries`. Each Bill section owns a `bill_<section>.jsonl` sibling file, a `bills.<section>_fetched_at` column, and a singular entity name in `validation_failures`. The catalogue `concord.models.bills.BILL_SECTIONS` is the single source of truth for those names — names and derived strings only, never per-stage behavior ([ADR 0025](docs/adr/0025-bill-section-catalogue-data-not-registry.md)). Not to be confused with **Section** (a Congressional Record grouping, above). Older prose and stats field names say "tier-2" for the same concept; new code says Bill section.
...
- **Daemon** — the long-lived, unsupervised supervisor started by `concord daemon` (see [ADR 0026](docs/adr/0026-unsupervised-scraping-daemon.md)). It does no scraping itself; it drives the stage CLI commands as child processes on a schedule. It owns *scheduling* and *backfill state*; a process supervisor (systemd, Docker) owns *liveness*.
- **Tick** — one scheduling iteration of the Daemon (default cadence: daily). A Tick drives all enabled entities through a **forward** pass plus up to `--backfill-per-tick` **backfill** chunks each, then their `load` + `index`.
- **Forward pass** — the per-Tick refresh of the present: the newest target Congress (with `--skip-unchanged`, per [ADR 0015](docs/adr/0015-staleness-aware-rescrape.md)) for Congress-scoped entities, and a trailing date-window ending today for Proceedings. Cheap; keeps the store current.
- **Backfill** — the per-Tick fill of the past, in **bounded chunks** walking history backward: one older Congress per chunk for Members/Bills/Votes, one `--proceedings-window`-day window per chunk for Proceedings (down to `--proceedings-since`). Completion is recorded in the **daemon state** so a finished chunk is never repeated.
- **Daemon state** — `daemon_state.json` in the data dir: the Daemon's backfill watermark (per-entity backfilled Congresses + the Proceedings cursor). Concord-originated operational state, the same category as the Scrape Run ledger — not rebuildable, not a mirror — stored as JSON to avoid a schema migration since only the Daemon reads it. See [ADR 0026](docs/adr/0026-unsupervised-scraping-daemon.md).
