...
- **One pipeline definition, shared.** Each entity's inline scrape→load→index chain (today duplicated inside every `run_<entity>_command`) is extracted into a `run_<entity>_pipeline(...)` function in that entity's CLI module; both `run <entity>` and `sync` call it. The Sync is a composition root in `cli/cycle.py` that calls the four explicitly — **not** a base class or protocol (which [ADR 0007](./0007-parallel-pipelines-per-entity.md) forbids).
